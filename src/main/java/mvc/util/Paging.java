...
public class Paging {
	...
	public Paging(int totalCount, int curPage,String title) {
		this(totalCount);
		this.setTitle(title);
		this.setCurPage(curPage);
	...
	private void makePaging() {
		...
		if(pageCount == 0)	setPageCount(6); //한 화면에 보이는 페이지수 기본값(10) 세팅
		...
		nextstartPage = startPage+10;
		if(curPage<11) {
			previousPage = startPage;
		}else {
			previousPage = nextstartPage-20;
		}
